...
> **玩家点击 NPC 默认进入稳定的对话域；Adapter 通过 Tool View、UI 默认值、点击重入门禁和 effect-time guard 保证对话由玩家自然推进和结束。**
...
8. Adapter ActionResult 日志记录 rejected / failed / cancelled 的 code 与 message。
9. 保持 Runtime Core game-agnostic，不新增 Runtime 对 Stardew capability name 的执行分支。
```
...
reply_options: string[], optional, max 4 items, max 80 chars each
...
- 有 reply_options 时显示所有合法 reply_options；
...
自由输入入口与生成选项是独立 UI affordance，不因为模型生成了 4 个选项而消失。
...
Source-time gate 位于 `PlayerInteractProbe`：

```text
- 游戏未 ready 时不发送事件；
- active menu 或 dialogueUp 时不发送事件；
- NPC invisible / sleeping 时不发送事件；
- 玩家不可行动时不发送事件；
...
ActionResult.code = interaction_context_changed / interaction_context_missing
...
player_interacted_with_npc suppressed: reason
ActionRequest received: capability, action_id, source_event_id, source_turn_id
ActionResult sent: status, code, message
...
- present_dialogue description 明确无回复入口表示结束对话；
...
```text
- ParsePresentDialogueInput 缺省 AllowFreeText=true；
- allow_free_text 显式 false 时关闭自由输入入口；
- reply_options 最多保留 4 个；
- 自由输入入口不占用 reply_options 数量上限；
- UI 同时展示 0-4 个 option 与自由输入入口；
...
- 4 个 reply_options + free text 可以同时进入 UI layout；
...
- suppressed player interaction 打印 reason。
```
...
8. source-time gate 与 effect-time guard 使用一致距离语义。
9. ActionResult 日志足以解释 rejected / failed / cancelled。
10. Runtime Core 没有新增 game-specific 执行策略。
11. 自动测试通过。
12. 实机 smoke 通过。
```
